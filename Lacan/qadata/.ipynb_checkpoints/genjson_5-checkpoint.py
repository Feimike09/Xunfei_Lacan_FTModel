...
# 设置日志
logging.basicConfig(filename='app5.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def generate_single_entry(text: str) -> Dict:
    ...
    - 让微调模型具有让句子添加后现代词汇从而变得晦涩难懂的能力
    - 用户输入某些后现代或哲学黑话段落，生成更复杂、更晦涩的表达，以实现对学术“黑话”的趣味性模仿和讽刺。
    例如：
    {{
      "instruction": "将'这个理论很难理解'改写成更晦涩的表达。",
      "input": "这个理论很难理解。",
      "output": "该理论的符号结构在主体的认知框架中呈现出高度的异质性，导致理解的复杂性。"
}}
...
if __name__ == "__main__":
    ...
    output_file = "../datasets/lacan5.json"  # 修改输出文件为JSON格式
